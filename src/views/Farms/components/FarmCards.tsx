...
const FarmCards: React.FC = () => {
  ...
  const defoIndex = farms.findIndex(
    ({ tokenSymbol }) => tokenSymbol === 'DEFO',
  )

  const defoPrice =
    defoIndex >= 0 && stakedValue[defoIndex]
      ? stakedValue[defoIndex].tokenPriceInWeth
      : new BigNumber(0)

  const BLOCKS_PER_YEAR = new BigNumber(2336000)
  const DEFO_PER_BLOCK = new BigNumber(100)
...
const FarmCard: React.FC<FarmCardProps> = ({ farm }) => {
  ...
  }, [defo, lpTokenAddress, account, setHarvestable])

  const poolActive = true // startTime * 1000 - Date.now() <= 0

  return (
    <StyledCardWrapper>
      {farm.tokenSymbol === 'DEFO' && <StyledCardAccent />}
      <Card>
        <CardContent>
          <StyledContent>
